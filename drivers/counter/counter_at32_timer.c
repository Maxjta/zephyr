...
static void at32_check_it_enabled(tmr_type *tmr_x, int id)
{
  uint32_t enabled = 0;
  switch(id)
  {
...
static uint32_t counter_at32_set_alarm(const struct device *dev, uint8_t chan,
				   const struct counter_alarm_cfg *alarm_cfg)
{
	const struct counter_at32_config *config = dev->config;
...
static int counter_at32_get_tim_clk(const struct device *dev, uint32_t *tim_clk)
{
  ...
  tmr_type *tmr_x = (tmr_type *)cfg->reg;
	...
	uint32_t bus_clk, apb_psc, crmcfg;
...
static const struct counter_driver_api counter_at32_driver_api = {
